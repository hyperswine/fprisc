/* jobs.h (platform/esp-idf) -- blocking host work, run off the harts.
 *
 * A platform library (wifi.c, bluetooth.c) whose calls can block -- an RPC to
 * the radio chip, a scan that takes seconds -- must not run them on a hart:
 * that would stall every actor on the core.  It describes the call as a job
 * and submits it; the broker task runs it and raises the slot's interrupt,
 * which the runtime delivers to the actor that bound it (std/job.fpr).
 *
...
#ifndef FPR_PLATFORM_JOBS_H
#define FPR_PLATFORM_JOBS_H
#include "fpr.h"
#include <stddef.h>

typedef struct fpr_job fpr_job_t;
struct fpr_job {
  char *(*run)(const fpr_job_t *j); /* on the broker task; answers malloc'd result text */
  int slot;
...
/* queue it: TAG(0), or TAG(1) when the queue is full */
