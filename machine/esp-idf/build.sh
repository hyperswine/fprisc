...
#   FP-RISC -> rv32 assembly (fprc --system=esp-idf), then ESP-IDF builds it
#   with the runtime and this machine layer (project/, an IDF project).
# Needs ESP-IDF 5.3 in the environment: source machine/esp-idf/env.sh (or
# ESP-IDF's own export.sh).  FPR_ESP_DEBUG=1 adds the boot markers, the
# per-second hart watch and the failed-allocation report.
...
mkdir -p "$OUT/gen"
rm -f "$OUT/gen"/*.s
( cd "$ROOT" && LC_ALL=C.UTF-8 ./fprc --system=esp-idf --prelude=core/prelude.fpr "$PROG" "$OUT/gen/prog.s" > "$OUT/compile.log" 2>&1 ) \
