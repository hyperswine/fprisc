...
- **posix**: a process. The command line, the environment, the three streams,
  files, the clock, an exit status (docs/2026-09-18-BASE.md). No register map: it used
...
- **posix on the esp-idf host** (`--host=esp-idf`; rv32, ESP32-P4): the same
  system, and the same shared facilities (files, descriptors, sockets, the
  readiness watcher, the Base environment), on an RTOS. A hart is a FreeRTOS
  ...
  trade-offs are in 2026-09-23-ESP-IDF.md.
