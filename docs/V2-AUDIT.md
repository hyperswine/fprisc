...
| --- | --- | --- | --- |
| 1 | `Sys.arena` leaves `bigfree` uninitialized | **FIXED** | `fpr_pool_init` exists (`hal/core/fpr.h:127-132`) and sets all four fields; all 7 creation sites call it (`runtime.c:65,978`, `actors.c:1371,1410`, `entry.c:209`, `proc_entry.c:160`).  The arena site carries the war story as a comment (`runtime.c:978-980`). |
...
| 7 | No fsync anywhere in the storage path | **OPEN** | `grep -rn "fsync\|fdatasync\|F_FULLFSYNC" hal/ qos/` returns **nothing**.  `store_call_locked` appends through buffered stdio and `fclose`s (`qos/portable/store.c:80-88`); `qos_blkraw_write` pwrites and returns (`hal/unix/blk_raw.c:103-111`).  DISK.txt's torn-write-rollback story is still a design, not a property. |
