...
of is [2026-09-19-PRELIM_BASE_LIBRARY_DESIGN.md](2026-09-19-PRELIM_BASE_LIBRARY_DESIGN.md); the older
inventory and tier plan is [2026-09-16-STD-PLAN.md](2026-09-16-STD-PLAN.md).
...
is an executable script ([2026-09-18-BASE.md](2026-09-18-BASE.md)). A module is a file; `use "std/x"` finds it
...
- **Every loop is a tail call** with an accumulator: a list of any length costs no
  stack. (Stacks grow anyway: [2026-09-19-BOUNDS.md](2026-09-19-BOUNDS.md).)
...
| `std/httpcore` | the same API without HTTPS, so it needs sockets and no processes: a board (`--host=esp-idf`) serves HTTP with it. `serveOn poller` waits for connections and request bytes on a Poller; an `https://` URL answers Err saying there is no TLS transport |
...
| `std/store` | a keyed collection kept by SHARD actors: `open put remove get count page search`. Each shard caches its part in memory, appends to its own file and compacts only that file when it is mostly superseded; a front actor routes writes and answers reads with a plan, so readers ask shards directly and a search scans every shard at once. See [2026-09-20-LIVE.md](2026-09-20-LIVE.md) |
| `std/live` | a server-driven UI over a websocket, the LiveView way: `serve serveCached serveProjected replay` (`serveProjected`: each session is sent only its `project sid model` and renders nothing when it is unchanged), durable fields from paths (`field`, `custom`, and `each` for a list kept one record per element), `Ev msg`, `Cmd msg`, `Sub msg`, `Policy`. Messages are the app's own type, carried by `@Msg`. See [2026-09-20-LIVE.md](2026-09-20-LIVE.md) |
...
`platform/esp-idf/`, for the posix system's esp-idf host). A program importing one links
...
`std/ble` has no Bluetooth stack in it). docs/2026-09-23-ESP-IDF.md has the design.
...
- **Sol:** the PURE modules now run in Sol unchanged (`tests/std/solstd.sol`:
  ...
  these immediate ones (docs/2026-08-25-TRANSACTION.md).
